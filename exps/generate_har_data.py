...
import os
import random
from exps.har_datasset_utils import *

random.seed(1)
np.random.seed(1)


def generate_har( raw_data_path, dataset_name):
    ...
    # download data
    if not os.path.exists(raw_data_path + 'rawdata/UCI HAR Dataset.zip'):
        os.system(
            f"wget https://archive.ics.uci.edu/ml/machine-learning-databases/00240/UCI%20HAR%20Dataset.zip -P {raw_data_path}rawdata/")
    if not os.path.exists(raw_data_path + 'rawdata/UCI HAR Dataset/'):
        os.system(f"unzip {raw_data_path}rawdata/'UCI HAR Dataset.zip' -d {raw_data_path}rawdata/")

    X, y = load_data_har(raw_data_path + 'rawdata/')
    statistic = []
    num_clients = len(y)
