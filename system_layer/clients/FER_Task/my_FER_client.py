...
class clientPLAG(Client):
    ...
    def train(self):
        ...
        for step in range(max_local_steps):
            for x, y in trainloader:
                ...
                gm = torch.cat([p.data.view(-1) for p in self.model.base.parameters()], dim=0)
                pm = torch.cat([p.data.view(-1) for p in self.client_u.base.parameters()], dim=0)
    ...
    def set_parameters(self, model, coef_self):
        for new_param, old_param in zip(model.parameters(), self.client_u.base.parameters()):
