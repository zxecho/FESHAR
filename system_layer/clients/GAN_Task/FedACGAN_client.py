...
# 该AGAN算法中，生成器G不生成图片样式，而是直接生成可供分类头进行分类的特征向量。
class clinet_ACGAN(GAN_client):
    ...
    def train(self, current_round):
        ...
        for step in range(max_local_steps):
            # 将特征提取器和分类器解冻
            self.frozen_net(["discriminator", "generator", 'classifier', 'extractor'], False)
            # self.model.train()
            ...
            self.frozen_net(["generator", "discriminator", 'classifier', 'extractor'], True)
